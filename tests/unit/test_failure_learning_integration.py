...
def _patch_get_session(monkeypatch):
    ...
    Also truncates the failure_history table before each test to ensure
    cross-test isolation when checking rates and counts.
    """
    ...
    # Truncate failure_history before each test to prevent cross-test contamination
    with engine.begin() as conn:
        try:
            conn.execute(text("TRUNCATE TABLE failure_history CASCADE"))
        except Exception:
