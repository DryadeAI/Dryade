...
class TestCallLlmWithTools:
    """Verify _call_llm passes tools correctly."""

    @pytest.mark.asyncio
    async def test_call_llm_passes_tools(self):
        """When tools are provided, call() should receive tools."""
        llm = make_mock_llm()
        provider = OrchestrationThinkingProvider(llm=llm)
        ...
        # tool_choice is no longer explicitly passed; the LLM provider decides
...
class TestOrchestrateThinkNativeTools:
    ...
    async def test_orchestrate_think_uses_native_tools(self):
        ...
        # 1 agent tool + 11 self-mod tools = 12 (Phase 167: always-inject)
