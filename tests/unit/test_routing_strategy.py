...
class TestWeakStrategy:
    ...
    def test_meta_hint_create_only(self):
        """Phase 167: WeakStrategy meta_hint returns only the unified `create` tool."""
        s = WeakStrategy()
        tools = _make_tools(10)
        # Add a `create` tool (Phase 167: replaces self_improve)
        tools.append(
            {
                "type": "function",
                "function": {
                    "name": "create",
                    "description": "Create agent/tool/skill",
                    "parameters": {"type": "object", "properties": {}},
                },
            }
        )
