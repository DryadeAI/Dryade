...
async def test_stream_final_answer_uses_merge_thinking_true():
    ...
    _stream_final_answer uses merge_thinking=False so that reasoning tokens
    go to the thinking panel (on_thinking) and content tokens go to the
    chat bubble (on_token). This prevents reasoning from leaking into the
    user-visible message. A fallback re-emits reasoning as content if the
    model puts the entire answer in reasoning_content.
    """
    ...
    # Verify merge_thinking=False was passed (reasoning goes to thinking panel,
    # not merged into the chat bubble content)
