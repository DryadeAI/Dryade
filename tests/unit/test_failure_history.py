...
All tests use PostgreSQL test database for isolation via patched get_session.
...
def _patch_get_session(monkeypatch):
    ...
    Truncates the failure_history table before each test so that rows
    from previous tests do not bleed into assertions that count records.
    """
    ...
    # Truncate before each test to guarantee isolation between tests that
    # share the same PostgreSQL database.
    with Session() as _cleanup_session:
        from core.database.models import FailureHistoryRecord

        _cleanup_session.query(FailureHistoryRecord).delete()
        _cleanup_session.commit()
...
def _make_store() -> FailureHistoryStore:
    """Create a store (uses patched in-memory session)."""
...
class TestFailureHistoryStore:
    ...
    def test_uses_in_memory_db(self) -> None:
        """Store can be created with :memory: for test isolation."""
        store = _make_store()
    ...
    def test_tool_error_stats(self) -> None:
        ...
        # Cast to float: PostgreSQL func.avg() returns Decimal, not float.
        assert abs(float(stats["avg_retries"]) - 2.25) < 0.01
        # avg duration: (200+200+200+400)/4 = 250
        assert abs(float(stats["avg_duration_ms"]) - 250.0) < 0.01
