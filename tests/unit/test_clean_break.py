"""Clean break verification tests (Phase 119.6).
...
class TestHandCraftedAgentLoading:
    ...
    def test_agent_directories_exist(self):
        for agent in self.AGENTS:
            agent_dir = Path(f"dryade-core/agents/{agent}")
            assert agent_dir.exists(), f"dryade-core/agents/{agent}/ missing"
            assert (agent_dir / "config.yaml").exists(), (
                f"dryade-core/agents/{agent}/config.yaml missing"
            )
            assert (agent_dir / "__init__.py").exists(), (
                f"dryade-core/agents/{agent}/__init__.py missing"
            )
...
class TestDryadeJsonManifests:
    ...
    def test_manifest_exists_and_valid_json(self, agent, framework):
        manifest_path = Path(f"dryade-core/agents/{agent}/dryade.json")
        assert manifest_path.exists(), f"dryade-core/agents/{agent}/dryade.json missing"
        data = json.loads(manifest_path.read_text())
        assert isinstance(data, dict)

    @pytest.mark.parametrize("agent,framework", AGENTS)
    def test_manifest_has_required_fields(self, agent, framework):
        data = json.loads(Path(f"dryade-core/agents/{agent}/dryade.json").read_text())
...
class TestMetaActionFactoryCreate:
    ...
    async def test_handle_meta_action_extracts_suggested_name(self):
        """Meta-action extracts agent name hint from message."""
