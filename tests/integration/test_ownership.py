...
def ownership_app():
    ...
    # Seed all users referenced by ownership tests (FK constraint on workflows.user_id)
    from core.database.models import User
    from core.database.session import get_session

    ownership_test_users = [
        ("owner-access-1", "owner-access-1@test.com"),
        ("owner-update-1", "owner-update-1@test.com"),
        ("owner-delete-1", "owner-delete-1@test.com"),
        ("deny-access-1", "deny-access-1@test.com"),
        ("deny-access-2", "deny-access-2@test.com"),
        ("deny-update-1", "deny-update-1@test.com"),
        ("deny-update-2", "deny-update-2@test.com"),
        ("deny-del-1", "deny-del-1@test.com"),
        ("deny-del-2", "deny-del-2@test.com"),
        ("admin-bypass-1", "admin-bypass-1@test.com"),
        ("admin1", "admin1@test.com"),
        ("admin-list-1", "admin-list-1@test.com"),
        ("admin-list-2", "admin-list-2@test.com"),
        ("admin-list", "admin-list@test.com"),
        ("public-view-1", "public-view-1@test.com"),
        ("public-view-2", "public-view-2@test.com"),
        ("public-list-1", "public-list-1@test.com"),
        ("public-list-2", "public-list-2@test.com"),
    ]

    with get_session() as session:
        for uid, email in ownership_test_users:
            existing = session.query(User).filter_by(id=uid).first()
            if not existing:
                session.add(
                    User(id=uid, email=email, password_hash=None, role="member", is_active=True)
                )
...
class TestOwnershipAccess:
    ...
    def test_owner_can_update_own_workflow(self, ownership_app):
        ...
        # Update as owner with unique name to avoid DB collision
        unique_update_name = f"Updated Name-{uuid.uuid4().hex[:8]}"
        update_response = client.put(
            f"/api/workflows/{workflow_id}", json={"name": unique_update_name}
        )
        assert update_response.status_code == 200
        assert update_response.json()["name"] == unique_update_name
