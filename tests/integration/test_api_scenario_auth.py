...
def scenario_client(integration_test_app):
    """Test client for scenario auth tests, reusing the session-scoped app."""
    from core.auth.dependencies import get_current_user

    def override_user_a():
        return {"sub": "user-a", "email": "a@test.com", "role": "user"}

    integration_test_app.dependency_overrides[get_current_user] = override_user_a
...
def seeded_executions(scenario_client):
    """Seed scenario execution records for user-a and user-b (idempotent)."""
    from core.database.models import ScenarioExecutionResult
    from core.database.session import get_session

    now = datetime.now(UTC)
    with get_session() as db:
        if not db.query(ScenarioExecutionResult).filter_by(execution_id="exec-user-a").first():
            db.add(
        ...
        if not db.query(ScenarioExecutionResult).filter_by(execution_id="exec-user-b").first():
            db.add(
                ScenarioExecutionResult(
