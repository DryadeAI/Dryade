"""
Shared pytest fixtures for Dryade tests.
...
# ---------------------------------------------------------------------------
# Test JWT secret — safe, well-known value for tests only.
# Must pass the validate_jwt_secret field validator (>=32 chars, no insecure
# substrings). Set as an env var so module-level imports that call
# get_settings() directly (e.g. TestMFARoutes) don't fail at collection time.
# ---------------------------------------------------------------------------
_TEST_JWT_SECRET = (
    "test-jwt-secret-safe-for-unit-tests-only-00000"  # 46 chars, no insecure patterns
)

os.environ.setdefault("DRYADE_JWT_SECRET", _TEST_JWT_SECRET)
os.environ.setdefault("DRYADE_LLM_BASE_URL", "http://localhost:8000/v1")
...
def _clear_engine_cache():
    """Ensure tests don't use cached production engine."""
...
def db_session(pg_engine):
    ...
    Yields:
        sqlalchemy.orm.Session: Database session backed by PostgreSQL.

    Note:
        Each test runs inside a transaction that is rolled back after
        the test completes, ensuring complete test isolation without
...
@pytest.fixture(autouse=True)
def mock_allowlist_watchdog(monkeypatch):
    """Prevent allowlist watchdog from starting during tests.

    The watchdog watches filesystem events which can interfere with
    test isolation. Tests that need the real watchdog should override
    this fixture.
    """
    mock_watchdog = MagicMock()
    mock_watchdog.start = AsyncMock()
    mock_watchdog.stop = AsyncMock()
    mock_watchdog.set_callback = MagicMock()
    mock_watchdog.is_running = False

    monkeypatch.setattr(
        "core.ee.allowlist_watchdog_ee.get_allowlist_watchdog",
        lambda: mock_watchdog,
    )
