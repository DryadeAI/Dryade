...
class TestKnowledgeUploadEndpoint:
    ...
    def test_upload_knowledge_unavailable(self, knowledge_client):
        """Test 501 when knowledge not available."""
        with patch("core.api.routes.knowledge.KNOWLEDGE_AVAILABLE", False):
    ...
    def test_upload_unsupported_format(self, knowledge_client):
        ...
        # Mock the crewai imports that happen before file type check
        mock_pdf_source = MagicMock()
        mock_text_source = MagicMock()

        with (
            patch("core.api.routes.knowledge.KNOWLEDGE_AVAILABLE", True),
            patch.dict(
                sys.modules,
                {
                    "crewai.knowledge.source": MagicMock(
                        PDFKnowledgeSource=mock_pdf_source, TextFileKnowledgeSource=mock_text_source
                    )
                },
            ),
        ):
...
class TestKnowledgeDeleteEndpoint:
    ...
    def test_delete_source_not_found(self, knowledge_client):
        """Test 404 when source not found."""
        with (
            patch("core.api.routes.knowledge.KNOWLEDGE_AVAILABLE", True),
            patch("core.api.routes.knowledge.get_knowledge_source_info", return_value=None),
        ):
...
class TestKnowledgeQueryEndpoint:
    ...
    def test_query_knowledge_success(self, knowledge_client):
        ...
        with (
            patch("core.api.routes.knowledge.KNOWLEDGE_AVAILABLE", True),
            patch("core.knowledge.embedder.get_crew_storage") as mock_storage,
        ):
            storage_instance = MagicMock()
            storage_instance.search.return_value = mock_results
    ...
    def test_query_knowledge_validation_error(self, knowledge_client):
        ...
        with patch("core.api.routes.knowledge.KNOWLEDGE_AVAILABLE", True):
            query_data = {"query": ""}  # Empty query should fail
            response = knowledge_client.post("/api/knowledge/query", json=query_data)
            assert response.status_code == 422

    def test_query_knowledge_with_source_filter(self, knowledge_client):
        """Test query with source_ids filter."""
        mock_results = []
