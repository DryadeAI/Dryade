...
def plans_client():
    ...
    if os.path.exists("./test_plans.db"):
        os.remove("./test_plans.db")
...
def sample_plan_data():
    ...
    return {
        ...
        "nodes": [
            {"id": "node1", "agent": "research", "task": "Research topic"},
            {"id": "node2", "agent": "writer", "task": "Write summary", "depends_on": ["node1"]},
        ],
...
@pytest.fixture
def created_conversation(plans_client):
    ...
    with get_session() as session:
        conv = Conversation(
            id="test-conv-123", user_id="test-user-plans", title="Test Conversation", mode="planner"
        )
        session.add(conv)
    ...
    with get_session() as session:
        session.query(Conversation).filter_by(id="test-conv-123").delete()
