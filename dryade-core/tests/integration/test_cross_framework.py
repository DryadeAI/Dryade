...
async def test_adk_adapter_session_persistence():
    ...
    # Patch ADK availability and imports
    with (
        patch("core.adapters.adk_adapter._ADK_AVAILABLE", True),
        patch("core.adapters.adk_adapter.Runner", return_value=mock_runner),
        patch(
            "core.adapters.adk_adapter.InMemorySessionService", return_value=mock_session_service
        ),
        patch(
            "core.adapters.adk_adapter.InMemoryArtifactService", return_value=mock_artifact_service
        ),
        patch("core.adapters.adk_adapter.genai_types") as mock_genai,
    ):
