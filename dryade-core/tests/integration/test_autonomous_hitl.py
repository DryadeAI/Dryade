...
class TestSandboxSkillExecution:
    ...
    @pytest.mark.asyncio
    async def test_skill_without_run_block_uses_llm(self):
        """Test that skills without run: block use LLM interpretation."""
        executor = RuntimeSkillExecutor()
        ...
        mock_llm = MagicMock()
        mock_llm.acall = AsyncMock(return_value={"content": "LLM interpretation result"})
        ...
        with patch.object(executor, "_get_llm", return_value=mock_llm):
            ...
            assert result == "LLM interpretation result"
