...
class TestValidationMiddleware:
    ...
    def _valid_result(self):
        """Create a real ValidationResult (not a PluginStub)."""
    ...
    def test_validation_passes(self):
        ...
        client = self._make_app()
        with patch(
            "core.api.middleware.validation.ValidationMiddleware._validate_request",
...
class TestAuthMiddleware:
    ...
    def test_valid_token(self):
        ...
        token = pyjwt.encode(
            {"sub": "user1", "role": "user", "exp": utcnow() + timedelta(hours=1)},
            secret,
            algorithm="HS256",
        )
    ...
    def test_expired_token(self):
        ...
        secret = "test-secret-key"
        token = pyjwt.encode(
            {"sub": "user1", "exp": utcnow() - timedelta(hours=1)},
            secret,
            algorithm="HS256",
        )
