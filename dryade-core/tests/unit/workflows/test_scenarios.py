...
class TestScenarioConfig:
    ...
    def test_invalid_domain_rejected(self, valid_config_dict):
        """ScenarioConfig rejects invalid domain values with a validation error."""
        config = valid_config_dict.copy()
        config["domain"] = "invalid_domain"
        with pytest.raises(ValidationError) as exc_info:
            ScenarioConfig(**config)
        error_str = str(exc_info.value)
        assert "invalid_domain" in error_str
        # Pydantic V2 uses 'Input should be ...' format instead of 'Must be one of ...'
