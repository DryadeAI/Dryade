...
class TestWebSocketAuthentication:
    ...
    async def test_authenticate_websocket_missing_token_returns_pending(self):
        """Verify missing token returns 'pending' (first-message auth path)."""
        mock_websocket = MagicMock()
        ...
        with patch("core.api.routes.websocket.get_settings") as mock_settings:
            ...
            # Missing token returns "pending" for first-message auth
            assert result == "pending"
            # Should NOT accept or close (first-message auth handles that)
