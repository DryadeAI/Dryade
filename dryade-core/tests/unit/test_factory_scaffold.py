...
class TestPathHelpers:
    ...
    def test_output_dir_default_uses_settings(self):
        """Default agent output dir uses settings.agents_dir (absolute)."""
        from core.config import get_settings

        result = get_output_dir(ArtifactType.AGENT, "my_agent")
        assert result == Path(get_settings().agents_dir) / "my_agent"
