...
1. Agent/tool/skill creation via unified `create` tool (Phase 167)
2. Agent creation (backward compat: EscalationExecutor._create_agent)
3. Tool creation (backward compat: EscalationExecutor._create_tool)
...
# ---------------------------------------------------------------------------
# Category 1: Unified `create` tool (Phase 167: replaces self_improve/create_agent/create_tool)
# ---------------------------------------------------------------------------

class TestAgentCreationUnified:
    """Phase 167: Unified `create` tool should build FACTORY_CREATE escalation."""
    ...
    @pytest.mark.asyncio
    async def test_create_with_agent_artifact_type(self):
        result = await execute_self_mod_tool(
            tool_name="factory_create",
            arguments={
                "name": "code_review",
        ...
        assert isinstance(result, PendingEscalation)
        assert result.action.action_type == EscalationActionType.FACTORY_CREATE
        assert result.action.parameters.get("artifact_type") == "agent"

    @pytest.mark.asyncio
    async def test_create_includes_suggested_name(self):
        result = await execute_self_mod_tool(
            tool_name="factory_create",
            arguments={
                "name": "my_agent",
...
class TestToolCreation:
    """Phase 167: `create` tool with artifact_type=tool builds FACTORY_CREATE escalation."""

    @pytest.mark.asyncio
    async def test_create_tool_type_builds_factory_create(self):
        result = await execute_self_mod_tool(
            tool_name="factory_create",
            arguments={
                "goal": "Create a JSON schema validator tool",
                "artifact_type": "tool",
                "name": "json_validator",
            },
    ...
    @pytest.mark.asyncio
    async def test_create_tool_includes_suggested_name(self):
        result = await execute_self_mod_tool(
            tool_name="factory_create",
            arguments={
                "goal": "Shorten URLs",
    ...
    @pytest.mark.asyncio
    async def test_create_tool_goal_in_params(self):
        result = await execute_self_mod_tool(
            tool_name="factory_create",
            arguments={
...
class TestApprovalFlow:
    ...
    @pytest.mark.asyncio
    async def test_factory_create_question_has_agent_factory_framing(self):
        # Phase 167: question uses "Agent Factory" framing
        result = await execute_self_mod_tool(
            tool_name="factory_create",
...
class TestFactoryPipelineApi:
    ...
    def test_creation_request_importable(self):
        try:
            from core.factory.models import ArtifactType, CreationRequest  # noqa: F401
